...
def rpv_contamination(N, T, RP, RPV ):
    '''
...
def poissfit(data,alpha=0.05):
    ...
    r = np.array(data).ravel()
    
    lambdahat = r.mean()
    
    lambdaci = [
        chi2.ppf(alpha/2, 2*lambdahat*len(r)) / (2*len(r)), 
        max(0.0,chi2.ppf(1-alpha/2, 2*lambdahat*len(r) + 2) / (2*len(r)))
    ]
    
    return lambdahat, tuple(lambdaci)
